...
#define FP16MM
// /usr/local/cuda-8.0/bin/nvcc fp16_example.cu -I/usr/local/cuda-8.0/include  -gencode arch=compute_61,code=sm_61  -L/usr/local/cuda-8.0/lib64  -lcublas
...
cublasStatus_t checkCublas(cublasStatus_t result)
{
  if (result != CUBLAS_STATUS_SUCCESS) {
    std::cout<<"CUDA Runtime Error: %s\n"<<cublasGetErrorString(result)<<std::endl;;
...
int main(int argc, char ** argv){
  ...
  std::cout<<half_to_float(d_C[1])<<std::endl;
